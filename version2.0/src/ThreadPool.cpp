...
//pthread_create()需要传入函数对象时，避免声明成员函数为静态成员函数
template <typename T>
void* func_threadpool_thread(void* pointer)
{
    T* ptr = static_cast<T*>(pointer);
    ptr->threadpool_thread();
    return NULL;
}

template <typename T>
void* func_adjust_thread(void* pointer)
{
    T* ptr = static_cast<T*>(pointer);
    ptr->adjust_thread();
    return NULL;
...
int ThreadPool::threadpool_create(int _Min_thr_num, int _Max_thr_num, int _Queue_max_size) {
    ...
    do {
        ...
        for(int i = 0; i < _Min_thr_num; i++) {
            pthread_create(&threads[i], NULL, func_threadpool_thread<ThreadPool>, this);
            // std::cout << "start thread" << (unsigned int)threads[i] << std::endl;
        }

        /* 创建管理者线程 */
        // std::cout << "create adjust_thread" << std::endl;
        if(int ret = pthread_create(&adjust_tid, NULL, func_adjust_thread<ThreadPool>, this) != 0) {
...
void * ThreadPool::threadpool_thread() {
    ThreadPoolTask Task;
    while(true) {
        pthread_mutex_lock(&lock);
        ...
        if(shutdown) {
            pthread_mutex_unlock(&lock);
            // pthread_detach(pthread_self());后面使用了pthread_jion,所以这里不能分离
            pthread_exit(NULL);
        }
        //任务出队操作
